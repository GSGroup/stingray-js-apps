...
function movable() {
	...
	this.damage = function(obj, hit) {
		//ui.log("damaged for " + hit);
		obj.hp -= hit;
		if (obj.hp <= 0) {
			ui.log(capitalize(this.blinded? "something": obj.name) + " dies.");
			if (obj.boss) {
				map.boss_killed = true;
				ui.log("You killed the monster, return to the surface!");
			}
		...
		if (roll(this.poison_chance)) {
			ui.log(obj.name + " has been poisoned!");
			obj.poisoned = rand(3, 10);
		}
		if (roll(this.blind_chance)) {
			ui.log(obj.name + " has been blinded!");
...
function hero() {
	...
	this.read = function(obj) {
		ui.log("You read " + obj.name + ".");
		this.unlink(obj);
		var name = obj.name;
		if (name.indexOf("healing") >= 0) {
			this.hp = this.max_hp();
			ui.log("You feel fully restored.");
		} else if (name.indexOf("vision") >= 0) {
			ui.log("You feel somewhat enlightened.");
			map.foreach(function(cell) {
					if (!cell.visited) {
						cell.visited = true;
						map.repaint(cell);
	...
	this.heal = function(n) {
		...
		if (hp != hp0) {
			ui.log("You restored " + (hp - hp0) + " health points.");
		}
		this.hp = hp;
	}
	...
	this.throw_obj = function(obj, dx, dy) {
		...
		ui.animate_throw(x, y, obj, cell.actor, text + '.');
	}
	...
	this.combine = function(obj1, obj2) {
		...
		ui.log("You tried to combine " + obj1.name + " and " + obj2.name + " and " + text);
		if (obj)
	...
	this.fix = function(obj) {
		...
		++map.fixed;
		ui.run_mini_game(size, this.level_cap(), 10 - size);
	}
	this.add_cash = function(g) {
		...
		if (this.level < this.MAX_LEVEL && this.cash >= cap) {
			++this.level;
			ui.log(rand(["Occasionally you feel stronger!", "You gained next level!"]));
			this.cash -= cap;
		}
	}
	...
	this.get_actions = function() {
		...
		for(var i = 0; i < objs.length; ) {
			...
			case "key": {
					this.inv.add(o, true);
					objs.splice(i, 1);
					ui.log("You picked up key to the next level.");
					map.repaint(cell);
					continue;
				}
			default:
		...
		var a = [];
		if (mask & PICKABLE)
			a.push(new hero_action(this, '1', 'Pickup', PICKABLE, this.pickup));
		if (mask & EDIBLE)
			a.push(new hero_action(this, '2', 'Eat', EDIBLE, this.eat));
		if (mask & DRINKABLE)
			a.push(new hero_action(this, '3', 'Drink', DRINKABLE, this.drink));
		if (mask & EDIBLE) {
			var t = new hero_action(this, '6', 'Throw', EDIBLE, this.throw_obj)
			t.need_dir = true;
			a.push(t);
			if (mask & DRINKABLE) {
				var ha = new hero_action(this, '4', 'Alchemy', EDIBLE | DRINKABLE, this.combine);
				ha.combine = true;
				a.push(ha);
			}
		}
		if (mask & READABLE)
			a.push(new hero_action(this, '5', 'Read', READABLE, this.read));
		if (mask & FIXABLE)
			a.push(new hero_action(this, '7', 'Fix', FIXABLE, this.fix));
		
		a.push(new hero_action(this, '8', 'Search', 0, this.search));
		a.push(new hero_action(this, '9', 'Cheat', 0, this.cheat));
		
		var ct = cell.type;
		if (ct == CELL_EXIT || ct == CELL_ENTRANCE)
			a.push(new hero_action(this, '0', 'Exit', 0, this.exit));
		return a;
	}
	...
	this.tick = function() {
		if (this.poisoned > 0) {
			ui.log(rand(["You are slowly dying.", "You feel sick."]));
...
function dungeon_map(width, height) {
	...
	this.generate = function() {
		...
		if (hero.level == hero.MAX_LEVEL) {
			...
			ui.log(" Follow!  But! follow only if ye be men of valor, " + 
		...
		if (roll(0.3)) {
			...
			ui.log("Your senses tell you about broken pipe on this floor!");
		}
	...
	this.generate_monster = function() {
		...
		//ui.log("monster level: " + level + " of " + monster_list.length + "-&gt;" + m_level);
	...
	this.up = function() {
		...
		if (l == 0) {
			...
			if (this.boss_killed && this.fixed >= this.pipes)
				...
				ui.log("You must fix all pipes(and probably kill someone), until then there's no way out!");
			return;
		}
		ui.log("you climbed up the stairs to the level " + l + ".");
		this.cells = levels[--l];
		this.level = l;
	...
	this.down = function() {
		...
		if (l >= levels.length && key == null) {
			ui.log("This door is locked. Find the key to unlock it.");
			return;
		}
		
		this.level = l;
		ui.log("you stepped down the stairs to the level " + (l + 1) + ".");
